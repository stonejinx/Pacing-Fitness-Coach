...
import 'package:running_coach/apps/heart_tracker/model/band_pass_filter.dart';
...
class PpgFilter {
  ...
  Stream<(int, double)> get heartRateStreamWithTimestamp async* {
    ...
    await for (final (timestamp, value) in filteredStream) {
      ...
      if (smoothedHr > 30 && smoothedHr < 220) {
        final int bpmTimestamp = peakTimestamps.last;
        yield (bpmTimestamp, smoothedHr);
      }
  ...
  Stream<double> get heartRateStream =>
      heartRateStreamWithTimestamp.map((event) => event.$2);
}
